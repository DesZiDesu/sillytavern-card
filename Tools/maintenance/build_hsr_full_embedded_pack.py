...
def build_worldbook() -> dict[str, Any]:
    ...
    entries = [copy.deepcopy(entry) for entry in main["entries"].values() if not obsolete(entry)]
    ...
    entries.extend(sanitize_legacy_mentions(entry) for entry in tracker["entries"].values())
    entries.sort(key=lambda entry: (int(entry.get("displayIndex", 0)), int(entry.get("uid", 0))))
    ...
    main["entries"] = {str(index): entry for index, entry in enumerate(entries)}
...
def write_readme() -> None:
    README_PATH.write_text(
        ...
        "Import **one card only**. Each card embeds the complete 81-entry lorebook and exactly five regex scripts: Scene, Header, Monologue, Dialogue, and Status.\n\n"
